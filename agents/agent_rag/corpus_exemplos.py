...
CORPUS_EXEMPLOS = [
    {
        ...
        "query": """SELECT count(*) FROM "Pessoas" WHERE tipo = 'FORNECEDOR';"""
    },
    {
        "pergunta": "Liste o nome e o documento de todos os clientes ativos.",
        "query": """SELECT razaosocial, documento FROM "Pessoas" WHERE tipo = 'CLIENTE' AND status = 'ATIVO';"""
    },
    {
        "pergunta": "Qual o valor total de todas as contas a pagar que estão pendentes?",
        "query": """SELECT SUM(valortotal) FROM "MovimentoContas" WHERE tipo = 'PAGAR' AND status = 'PENDENTE';"""
    },
    {
        "pergunta": "Quanto eu tenho para receber no total?",
        "query": """SELECT SUM(valortotal) FROM "MovimentoContas" WHERE tipo = 'RECEBER';"""
    },
    {
        "pergunta": "Quais são as classificações de despesa disponíveis?",
        "query": """SELECT descricao FROM "Classificacao" WHERE tipo = 'DESPESA';"""
    },
    {
    ...
    },
    {
        ...
        "query": """SELECT documento FROM "Pessoas" WHERE razaosocial = 'Nome Exato da Pessoa';"""
    },
    {
        "pergunta": "Liste todas as parcelas pendentes, ordenadas por vencimento.",
        "query": """SELECT identificacao, valorparcela, datavencimento FROM "ParcelasContas" WHERE statusparcela = 'PENDENTE' ORDER BY datavencimento ASC;"""
    },
    {
        "pergunta": "Quais foram os últimos 5 movimentos do tipo 'PAGAR'?",
        "query": """SELECT numeronotafiscal, valortotal, dataemissao FROM "MovimentoContas" WHERE tipo = 'PAGAR' ORDER BY dataemissao DESC LIMIT 5;"""
    },
    {
        "pergunta": "Quem é o fornecedor da nota fiscal 'NF-123'?",
        "query": """SELECT T1.razaosocial FROM "Pessoas" T1 JOIN "MovimentoContas" T2 ON T1.id = T2."Pessoas_idFornecedorCliente" WHERE T2.numeronotafiscal = 'NF-123';"""
    },
    {
    ...
    },
    {
        ...
        "query": """SELECT razaosocial, status FROM "Pessoas" WHERE status = 'INATIVO';"""
    },
    {
        "pergunta": "Qual o valor médio dos movimentos a pagar?",
        "query": """SELECT AVG(valortotal) FROM "MovimentoContas" WHERE tipo = 'PAGAR';"""
    },
    {
        "pergunta": "Quais as parcelas do movimento da nota 'NF-456'?",
        "query": """SELECT T1.identificacao, T1.valorparcela FROM "ParcelasContas" T1 JOIN "MovimentoContas" T2 ON T1."MovimentoContas_idMovimentoContas" = T2.id WHERE T2.numeronotafiscal = 'NF-456';"""
    },
    {
    ...
    },
    {
        ...
        "query": """SELECT numeronotafiscal, valortotal, dataemissao FROM "MovimentoContas" WHERE tipo = 'RECEBER' AND dataemissao BETWEEN '2024-10-01' AND '2024-10-31';"""
    },
    {
    ...
    },
    {
        ...
        "query": """SELECT T2.id, T2.numeronotafiscal FROM "Classificacao" T1 JOIN "MovimentoContas_has_Classificacao" T_JOIN ON T1.id = T_JOIN."Classificacao_idClassificacao" JOIN "MovimentoContas" T2 ON T_JOIN."MovimentoContas_idMovimentoContas" = T2.id WHERE T1.descricao = 'INSUMOS AGRÍCOLAS';"""
    },
    {
        "pergunta": "Temos parcelas vencidas que ainda estão pendentes?",
        "query": """SELECT identificacao, valorparcela, datavencimento FROM "ParcelasContas" WHERE statusparcela = 'PENDENTE' AND datavencimento < CURRENT_DATE;"""
    },
    {
